############################################################
# Figure 4 replot from existing DPRS / ML results
# No model retraining, no DEG, no Mime1 rerun
############################################################
...
FIG4_DIR <- "D:/R_workspace/评分/AD_DDR_project/04-5_Mime1Matched_StepCoxFixed"

OUT_DIR <- file.path(FIG4_DIR, "Fig4_replot_final")
...
cindex_file <- file.path(FIG4_DIR, "Fig4B_all_model_Cindex_summary.csv")
selected_file <- file.path(FIG4_DIR, "Fig4B_final_selected_model_info.csv")
score_file <- file.path(FIG4_DIR, "Fig4C_DPRS_all_sets.csv")
gene_file <- file.path(FIG4_DIR, "Fig4C_final_signature_genes.csv")
...
save_csv(
  cindex_summary,
  file.path(TAB_DIR, "Fig4_replot_Cindex_summary.csv")
)

############################
# 4. C-index heatmap
# ComplexHeatmap + blue-white-red + black border + C-index text
# Columns: Training / Testing / GSE72094 / GSE68465 / Average
############################
...
plot_cindex_heatmap_final <- function(cindex_summary,
  ...
  # 这里只保留四个队列，不再把 Average 放进热图主体
  mat <- dat %>%
    dplyr::select(
      dplyr::all_of(dataset_cols)
    ) %>%
    as.matrix()
  ...
  # 右侧 Average C-index 横条
  # Average 不在热图主体里显示，只在这里显示
  ...
  avg_bar_anno <- ComplexHeatmap::AnnotationFunction(
    ...
    fun = function(index, k, n) {
      ...
      # ComplexHeatmap 行从上到下；grid y 坐标从下到上，所以要反过来
  ...
  ht <- ComplexHeatmap::Heatmap(
    ...
    column_title = "Machine-learning model ",
...
############################
# 5. Average C-index barplot
############################

top_bar <- clean_cindex_summary(
  cindex_summary = cindex_summary,
  best_model = best_model,
  dataset_cols = dataset_cols
) %>%
...
############################
# 6. Read DPRS score and risk groups
############################
...
final_score <- final_score %>%
  ...
  dplyr::rename(ML_DDRscore = DPRS)

train_tmp <- final_score %>% dplyr::filter(Dataset == "Training")

if (nrow(train_tmp) > 30 && sum(train_tmp$status == 1) > 5) {
  fit_dir <- survival::coxph(
    survival::Surv(time, status) ~ ML_DDRscore,
    data = train_tmp
  )
  
  if (as.numeric(coef(fit_dir)[1]) < 0) {
...
final_score$Dataset <- factor(final_score$Dataset, levels = dataset_cols)
...
############################
# 7. Risk score and survival status distribution
############################
...
if (requireNamespace("timeROC", quietly = TRUE)) {
  ...
  save_csv(
    auc_all,
    file.path(TAB_DIR, "Fig4E_DPRS_timeROC_AUC.csv")
  )
...
############################
# 10. Final signature gene coefficient plot
############################
...
sink(file.path(OUT_DIR, "Fig4_replot_session_info.txt"))
cat("Figure 4 replot finished.\n")
cat("Input directory:", FIG4_DIR, "\n")
cat("Output directory:", OUT_DIR, "\n")
cat("Selected model:", best_model, "\n\n")
print(sessionInfo())
sink()

cat("\nFigure 4 replot finished.\n")
cat("Output directory:\n", OUT_DIR, "\n")
cat("\nMain output figures:\n")
cat(" - fig/Fig4B_Cindex_heatmap_top15.pdf\n")
cat(" - fig/Fig4B_Cindex_heatmap_top30.pdf\n")
cat(" - fig/Fig4B_Cindex_heatmap_all_models.pdf\n")
cat(" - fig/Fig4B_average_Cindex_barplot_top30.pdf\n")
cat(" - fig/Fig4C_DPRS_risk_score_distribution.pdf\n")
cat(" - fig/Fig4C_DPRS_survival_status_distribution.pdf\n")
cat(" - fig/Fig4D_DPRS_KM_*.pdf\n")
cat(" - fig/Fig4E_DPRS_timeROC_*.pdf\n")
cat(" - fig/Fig4E_DPRS_timeROC_AUC_summary.pdf\n")
cat(" - fig/Fig4F_DPRS_signature_gene_lollipop.pdf\n")