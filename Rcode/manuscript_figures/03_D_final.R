...
feature_order <- c("Exclusion score", "CAF", "Endothelial cells", "Fibroblasts", "TGF-beta signaling", "EMT")
...
map_feature <- function(feature) {
  ...
  dplyr::case_when(
    grepl("EXCLUSION", f) ~ "Exclusion score",
    grepl("^CAF$|CANCER.?ASSOCIATED.?FIBRO", f) ~ "CAF",
    grepl("ENDOTHELIAL", f) ~ "Endothelial cells",
    grepl("FIBROBLAST", f) & !grepl("CAF|CANCER.?ASSOCIATED", f) ~ "Fibroblasts",
    grepl("TGF|TGFB", f) ~ "TGF-beta signaling",
    grepl("^EMT$|EPITHELIAL.*MESENCHYMAL", f) ~ "EMT",
    TRUE ~ NA_character_
  )
}
...
fwrite(plot_df, file.path(DATA_DIR, "Figure3D_immune_exclusion_full_plot_data.csv"))
fwrite(main_df, file.path(DATA_DIR, "Figure3D_immune_exclusion_main_plot_data.csv"))
fwrite(
  data.frame(Cancer = main_cancer_order),
  file.path(DATA_DIR, "Figure3D_immune_exclusion_selected_cancers.csv")
)

make_heatmap(
  main_df, main_cancer_order,
  "D  Immune-exclusion features",
  "Figure3D_immune_exclusion",
  FIG_DIR, width = 12.2, height = 5.9
)

make_heatmap(
  plot_df, full_cancer_order,
  "Figure S3D  Immune-exclusion programs",
  "FigureS3D_immune_exclusion_full_heatmap",
  SUP_DIR, width = 18.5, height = 6.2, star_size = 3.25
)

legend_txt <- c(
  "Figure 3D. Immune-exclusion programs across MO-DDRscore-defined tumor states.",
  ...
  "",
  "Figure S3D. Full pan-cancer immune-exclusion program landscape across all available cancer types."
)
writeLines(legend_txt, file.path(DATA_DIR, "Figure3D_legend.txt"), useBytes = TRUE)

cat("Done: Figure3D immune-exclusion programs\n")
cat("Main output:", file.path(FIG_DIR, "Figure3D_immune_exclusion.png"), "\n")
cat("Supplementary output:", file.path(SUP_DIR, "FigureS3D_immune_exclusion_full_heatmap.png"), "\n")
